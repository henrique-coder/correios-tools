...
export function openImageViewer(obj: string, dh: string): void {
  ...
  m.innerHTML = `<div style="background:#1e293b;padding:16px;border-radius:12px;position:relative;min-width:300px;min-height:300px;max-width:85vw;max-height:85vh;display:flex;flex-direction:column;box-shadow:0 25px 50px -12px rgba(0,0,0,.5);border:1px solid #334155">
    ...
    <div style="position:absolute;bottom:24px;right:24px;display:flex;gap:12px;z-index:11">
      ...
      <button id="${id}-rotR" style="background:rgba(15,23,42,.85);color:#fff;border:1px solid rgba(255,255,255,.2);border-radius:12px;width:54px;height:54px;cursor:pointer;font-size:24px" title="Rotacionar Direita">↻</button>
    </div>
    <div id="${id}-ld" style="padding:50px;text-align:center;font-weight:bold;color:#60a5fa;font-size:15px;flex:1;display:flex;align-items:center;justify-content:center">Buscando imagem no servidor...</div>
    <div style="overflow:hidden;border-radius:8px;display:flex;align-items:center;justify-content:center;background:#0f172a;flex:1;min-height:200px">
      <img src="${url}" id="${id}-img" style="max-width:100%;max-height:calc(85vh - 32px);object-fit:contain;transition:transform .15s ease-out;display:none;transform-origin:center"
        ...
        onerror="document.getElementById('${id}-ld').innerHTML='⚠️<br><br>Imagem inexistente ou indisponível.';document.getElementById('${id}-ld').style.color='#ef4444'">
    </div>
  </div>`;
